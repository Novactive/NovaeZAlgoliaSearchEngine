...
All basic cases that are used by Ez Platform to retrieve the contents and locations are managed by this bundle behind the scene. 
...
The User related criterions are not implemented because they are either included in the UserMetaData Criterion or not relevant due to the default content types limitations.
...
- _FullText_ Criterion cannot be inside _LogicalNot_ or LogicalOr operator because it's moved to the query string of the Algolia request which is performed anyway.
...
### Sorting
To be able to sort the documents retrieved from Algolia via Search method you can use the Replicas.
...
The source code of the Front End implementation with React components can be found in the [`search.jsx`](https://github.com/Novactive/NovaeZAlgoliaSearchEngine/blob/master/bundle/Resources/assets/js/search.jsx) file.
